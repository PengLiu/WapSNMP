...
func (w WapSNMP) ParseTrap(response []byte) (map[string]interface{}, error) {
	...
	if snmpVer < 3 {
		...
		*/
		...
		if len(respAuthParam) == 0 || len(respPrivParam) == 0 {
			return result, errors.New("response is not encrypted")
		}
		if len(w.Trapusers) == 0 {
			return result, errors.New("No SNMP V3 trap user configured")
		}
		...
		if !founduser {
			return result, errors.New("No matching user found")
		}
